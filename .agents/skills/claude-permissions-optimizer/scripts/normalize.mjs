...
export function isRiskFlag(token, base) {
  if (GLOBAL_RISK_FLAGS.has(token)) {
    return true;
  }
  ...
  const contexts = Object.hasOwn(CONTEXTUAL_RISK_FLAGS, token)
    ...
    : undefined;
  if (contexts && base && contexts.has(base)) {
    return true;
  }
  // Combined short flags containing risk chars: -rf, -fr, -fR, etc.
  if (/^-[a-zA-Z]*[rf][a-zA-Z]*$/.test(token) && token.length <= 4) {
    return true;
  }
  return false;
}
...
export function normalize(command) {
  ...
  if (/\|\s*(sh|bash|zsh)\b/.test(command)) {
    return command;
  }
  ...
  if (pnpmFilter) {
    return "pnpm --filter * " + pnpmFilter[1] + " *";
  }
  ...
  if (/^find\s/.test(command)) {
    if (/\s-delete\b/.test(command)) {
      return "find -delete *";
    }
    if (/\s-exec\s/.test(command)) {
      return "find -exec *";
    }
