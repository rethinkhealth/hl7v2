...
function matchGlob(pattern, command) {
  ...
  if (normalized.endsWith(" *")) {
    ...
    regexStr =
      "^" +
      normalized.replaceAll(/[.+^${}()|[\]\\]/g, "\\$&").replaceAll("*", ".*") +
      "$";
  }
  try {
...
function classify(command) {
  ...
  if (compoundMatch) {
    return classify(compoundMatch[1].trim());
  }
  ...
  for (const { test, reason } of RED_PATTERNS) {
    if (test.test(command)) {
      return { tier: "red", reason };
    }
  }

  // GREEN checks
  const baseCmd = command.split(/\s+/)[0];
  if (GREEN_BASES.has(baseCmd)) {
    return { tier: "green" };
  }
...
for (const [pattern, data] of patternGroups) {
  ...
  const cmd = pattern.replaceAll(/^Bash\(|\)$/g, "");
...
for (const [pattern, data] of patternGroups) {
  switch (data.tier) {
    ...
    case "yellow": {
      yellowCount++;
      yellowNames.push(
        pattern.replaceAll(/^Bash\(|\)$/g, "").replace(/ \*$/, "")
      );
      break;
    }
    case "red": {
      redBlocked.push({
        pattern: pattern.replaceAll(/^Bash\(|\)$/g, ""),
        reason: data.reason,
        count: data.totalCount,
      });
      break;
