...
const myVersionSpecificPlugin: Plugin<[], Root, Root> = () => {
  return (tree: Root) => {
    ...
    if (!info) {
      throw new Error(
        "Message not annotated. Use hl7v2AnnotateMessage plugin first."
      );
    }

    console.log(
      `Processing ${info.messageCode}^${info.triggerEvent} v${info.version}`
    );
    ...
    .use(function versionGuard() {
      return (tree: Root) => {
        if (tree.data?.messageInfo?.version !== version) {
          throw new Error(
            `Expected version ${version}, got ${tree.data?.messageInfo?.version}`
          );
        }
        return tree;
      };
