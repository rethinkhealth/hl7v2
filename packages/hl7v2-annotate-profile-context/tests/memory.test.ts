 ...
 *
 * Verifies that:
 * 1. Profile Maps on file.data hold references to shared LRU-cached objects, not copies
 ...
 * 4. GC reclaims file.data Maps after processing
 */
...
describe("memory: shared references", () => {
  it("file.data.fields entries are the same object references as the LRU cache", async () => {
    ...
    expect(file.data.fields!.get("MSH")).toBe(cachedMsh);
    expect(file.data.fields!.get("PID")).toBe(cachedPid);
  });

  it("file.data.datatypes entries are the same object references as the LRU cache", async () => {
    const tree = m(msh("2.5"), pid());
    const file = new VFile();
    await processor.run(tree, file);

    const cachedSt = await profiles.datatypes.load("2.5", "ST");
    expect(file.data.datatypes!.get("ST")).toBe(cachedSt);
  });

  it("file.data.tables entries are the same object references as the LRU cache", async () => {
    const tree = m(msh("2.5"), pid());
    const file = new VFile();
    await processor.run(tree, file);

    // Find any table entry and verify it matches the cache
    const firstTableId = [...file.data.tables!.keys()][0]!;
    const cachedTable = await profiles.tables.load("2.5", firstTableId);
    expect(file.data.tables!.get(firstTableId)).toBe(cachedTable);
  });
  ...
  it("two messages share the same profile object references", async () => {
    ...
    expect(file1.data.fields).not.toBe(file2.data.fields);
    expect(file1.data.fields!.get("MSH")).toBe(file2.data.fields!.get("MSH"));
    expect(file1.data.fields!.get("PID")).toBe(file2.data.fields!.get("PID"));
    expect(file1.data.datatypes!.get("ST")).toBe(
      file2.data.datatypes!.get("ST")
    );
  });
});

describe("memory: per-message overhead", () => {
  it("file.data Maps contain only index entries, not profile data copies", async () => {
    ...
    expect(file.data.fields!.size).toBe(3); // MSH, PID, OBX
    // Datatypes and tables are derived from those 3 segment definitions
    expect(file.data.datatypes!.size).toBeGreaterThan(0);
  ...
  it("Map sizes scale with unique segment types, not total segment count", async () => {
    ...
    expect(largeFile.data.fields!.size).toBe(smallFile.data.fields!.size);
    expect(largeFile.data.datatypes!.size).toBe(smallFile.data.datatypes!.size);
    expect(largeFile.data.tables!.size).toBe(smallFile.data.tables!.size);
  });
});
...
describe("memory: sustained load", () => {
  ...
  it("retaining file references accumulates only Map overhead, not profile data", async () => {
    ...
    const firstMsh = files[0]!.data.fields!.get("MSH");
    for (const file of files) {
      expect(file.data.fields!.get("MSH")).toBe(firstMsh);
    }
