 ...
 *
 * Verifies that:
 * 1. Profile Maps on file.data.profile hold references to shared LRU-cached objects, not copies
 ...
 * 4. GC reclaims file.data.profile Maps after processing
 */
...
describe("memory: shared references", () => {
  it("profile.fields entries are the same object references as the LRU cache", async () => {
  ...
  it("profile.datatypes entries are the same object references as the LRU cache", async () => {
    const tree = m(msh("2.5"), pid());
    const file = new VFile();
    await processor.run(tree, file);
  ...
  it("profile.tables entries are the same object references as the LRU cache", async () => {
    const tree = m(msh("2.5"), pid());
    const file = new VFile();
    await processor.run(tree, file);
...
describe("memory: per-message overhead", () => {
  it("profile Maps contain only index entries, not profile data copies", async () => {
    ...
    expect(file.data.profile!.fields.size).toBe(3); // MSH, PID, OBX
