...
describe("HL7v2 - Generate", () => {
    ...
    it("should generate MSH segment with default delimiters", () => {
      ...
      console.log(result);
      expect(result).toBe(
        "MSH|^~\\&|SIMHOSP|SFAC|RAPP|RFAC|20200508130643||ADT^A01|5|T|2.3\r"
      );
    });
    ...
    it("should handle components correctly", () => {
      ...
      const result = generator.generate(json);
      expect(result).toBe(
        "PID|1||PATID1234^5^M11^ADT1^MR^GOOD HEALTH HOSPITAL||SMITH^JOHN\r"
      );
    });
    ...
    it("should handle segment repetition", () => {
      ...
      const result = generator.generate(json);
      expect(result).toBe(
        "NK1|1|NUCLEAR^NELDA^W\rNK1|2|NUCLEAR^NELDA^W\r"
      );
    });
    ...
    it("should throw error when MSH.1 is not exactly one character", () => {
      ...
      expect(() => generator.generate(json)).toThrow("MSH.1 (field separator) must be exactly one character");
    });
    ...
    it("should throw error when MSH.2 is not exactly four characters", () => {
      ...
      expect(() => generator.generate(json)).toThrow("MSH.2 (encoding characters) must be exactly four characters");
    });
